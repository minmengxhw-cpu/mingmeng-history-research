...
def intake_report_status(counts: Counter[str]) -> str:
    """Expose a stable top-level status while preserving per-target counts."""
    if not counts:
        return "NOT_RUN"
    if len(counts) == 1:
...
def main() -> int:
    ...
    report = {
        ...
        "status": intake_report_status(counts),
