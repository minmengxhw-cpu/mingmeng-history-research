...
from scripts.domestic.validate_research_tracks import validate as validate_tracks  # noqa: E402
from scripts.domestic.validate_academic_layer import validate as validate_academic_layer  # noqa: E402
from scripts.domestic.audit_academic_formal_coverage import audit as audit_academic_formal_coverage  # noqa: E402
from scripts.domestic.audit_academic_source_reuse import audit as audit_academic_source_reuse  # noqa: E402
from scripts.domestic.validate_academic_gar639_page_identity_candidate import (  # noqa: E402
    validate as validate_academic_gar639_page_identity_candidate,
)
from scripts.domestic.validate_academic_acquisition_reconciliation import (  # noqa: E402
    validate as validate_academic_acquisition_reconciliation,
)
...
ACADEMIC_SOURCE_REUSE_MAP_PATH = ROOT / "data" / "domestic" / "academic_formal_reuse_map.json"
ACADEMIC_GAR639_PAGE_IDENTITY_CANDIDATE_PATH = ROOT / "data" / "domestic" / "academic_gar639_page_identity_candidate.json"
ACADEMIC_RECONCILIATION_PATH = ROOT / "data" / "domestic" / "academic_acquisition_reconciliation.json"
...
def candidate_alignment_check(db_path: Path) -> dict[str, Any]:
    """Ensure formal candidates align while allowing explicit discovery leads.

    Metadata-only discovery records are intentionally allowed to remain outside
    the formal SQLite index until an authorized intake supplies a local file,
    provenance, and review decision.  Every record remains visible in the
    JSONL queue, and a discovery record is still an error if it appears in the
    formal table prematurely.
    """
    records = read_candidate_records(CANDIDATES_PATH)
    file_ids = {str(record["candidate_id"]) for record in records}
    discovery_only_ids = {
        str(record["candidate_id"])
        for record in records
        if record.get("formal_candidate", True) is False
    }
    formal_file_ids = file_ids - discovery_only_ids
    connection = sqlite3.connect(f"file:{db_path}?mode=ro", uri=True)
    db_ids = {str(row[0]) for row in connection.execute("SELECT candidate_id FROM domestic_candidates")}
    connection.close()
    missing = sorted(formal_file_ids - db_ids)
    extra = sorted(db_ids - formal_file_ids)
    discovery_only_in_db = sorted(discovery_only_ids & db_ids)
    return {
        "status": "PASS" if not missing and not extra and not discovery_only_in_db else "FAIL",
        "file_count": len(file_ids),
        "formal_file_count": len(formal_file_ids),
        "discovery_only_count": len(discovery_only_ids),
        "db_count": len(db_ids),
        "missing_from_db": missing,
        "extra_in_db": extra,
        "discovery_only_in_db": discovery_only_in_db,
    }
...
def academic_layer_check() -> dict[str, Any]:
    ...
    academic_crosswalk_eligible_count = sum(
        record.get("academic_crosswalk_eligible", True) is not False
        for record in metadata_records
    )
    return {
        ...
        "academic_crosswalk_eligible_records": academic_crosswalk_eligible_count,
...
def public_surface_check() -> dict[str, Any]:
    ...
    routes = {
        ...
        "/research/tracks": app.research_tracks_page,
...
def build_report() -> dict[str, Any]:
    ...
    checks = {
        ...
        "academic_layer_quality": validate_academic_layer(),
        "academic_formal_coverage": audit_academic_formal_coverage(
            db_path, ACADEMIC_FULLTEXT_QUEUE_PATH, ACADEMIC_SOURCE_REUSE_MAP_PATH
        ),
        "academic_source_reuse": audit_academic_source_reuse(
            db_path, ACADEMIC_FULLTEXT_QUEUE_PATH, ACADEMIC_SOURCE_REUSE_MAP_PATH
        ),
        "academic_gar639_page_identity_candidate": validate_academic_gar639_page_identity_candidate(
            db_path, ACADEMIC_SOURCE_REUSE_MAP_PATH, ACADEMIC_GAR639_PAGE_IDENTITY_CANDIDATE_PATH
        ),
        "academic_acquisition_reconciliation": validate_academic_acquisition_reconciliation(
            ACADEMIC_RECONCILIATION_PATH, ACADEMIC_METADATA_INDEX_PATH
        ),
        ...
        "research_tracks": validate_tracks(RESEARCH_TRACKS_PATH, COVERAGE_PATH, CANDIDATES_PATH),
