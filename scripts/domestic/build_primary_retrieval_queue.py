...
def must_keep_route(row: dict[str, Any]) -> bool:
    ...
    references, public official-archive items, and exact official-compilation
    page leads are more useful for closing a primary gap than another duplicate
    public article, so they must remain visible in the metadata-only queue.
    This does not promote the route or read its body.
    """
    return bool(
        ...
        and (
            ...
            or (
                row.get("route_status") == "PUBLIC_ITEM_CANDIDATE"
                and str(row.get("repository_code") or "") in {"NAA-TW"}
            )
            or (
