...
def main() -> int:
    ...
    try:
        counts = {
            ...
            "observer_front_documents": conn.execute("select count(*) from documents where doc_key like 'domestic-ocr/NLC:observer-1947-v3n%-front-ocr'").fetchone()[0],
            "observer_front_pages": conn.execute("select count(*) from pages p join documents d on d.id=p.document_id where d.doc_key like 'domestic-ocr/NLC:observer-1947-v3n%-front-ocr'").fetchone()[0],
    ...
    result = {
        ...
        "observer": {"issues": len(observer), "derived_pdfs": sum((ROOT / row["derived_issue_pdf"]).is_file() for row in observer), "front_ocr_markdown": len(front_ocr), "db_documents": counts["observer_front_documents"], "db_pages": counts["observer_front_pages"]},
        "acceptance": {
    ...
    OUT_MD.write_text(
        "\n".join(
            [
                ...
                f"- 《观察》封面/目录 review-only 入库：{c['observer_front_documents']} 期文档、{c['observer_front_pages']} 页",
                ...
                "## 《观察》",
                "",
                f"- 已确认并切分 12 期，覆盖卷3第1—12期；首轮封面/目录 OCR 产出 {len(front_ocr)} 个 Markdown，并以 review-only 方式入库。",
