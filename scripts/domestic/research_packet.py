...
def build_research_packet(event_id: str) -> dict[str, Any] | None:
    ...
    for raw_track in topic.get("research_tracks", []):
        if not isinstance(raw_track, dict):
            continue
        lead_refs: list[dict[str, str]] = []
        for raw_lead in raw_track.get("lead_refs", []):
            if not isinstance(raw_lead, dict):
                continue
            lead_refs.append(
                {
                    "lead_id": str(raw_lead.get("lead_id") or ""),
                    "title": str(raw_lead.get("title") or ""),
                    "evidence_level": str(raw_lead.get("evidence_level") or ""),
                    "role": str(raw_lead.get("role") or ""),
                    "url": str(raw_lead.get("url") or ""),
                }
            )
        research_tracks.append(
            {
                "track_id": str(raw_track.get("track_id") or ""),
                "event_id": str(raw_track.get("event_id") or event_id),
                "track_role": str(raw_track.get("track_role") or ""),
                "title": str(raw_track.get("title") or ""),
                "period": str(raw_track.get("period") or ""),
                "status": str(raw_track.get("status") or ""),
                "primary_evidence_closed": raw_track.get("primary_evidence_closed") is True,
                "research_question": str(raw_track.get("research_question") or ""),
                "candidate_ids": [
                    str(value)
                    for value in raw_track.get("candidate_ids", [])
                    if str(value).strip()
                ],
                "lead_refs": lead_refs,
                "evidence_boundary": str(raw_track.get("evidence_boundary") or ""),
                "processing_policy": str(raw_track.get("processing_policy") or ""),
                "next_action": str(raw_track.get("next_action") or ""),
                "body_text_included": False,
            }
        )
    ...
    return {
        ...
        "audit": {
            ...
            "research_track_count": len(research_tracks),
            "research_tracks_body_text_included": False,
...
<article class="result compact-result"><div>
  ...
  <ul>{"".join(source_rows)}</ul>
    ...
    track_cards = []
    for track in packet.get("research_tracks", []):
        candidate_links = " · ".join(
            f'<a href="/domestic/candidate/{app.quote(str(candidate_id), safe="")}">{esc(candidate_id)}</a>'
            for candidate_id in track.get("candidate_ids", [])
        ) or "暂无候选回链"
        lead_links = " · ".join(
            f'<a href="{esc(app.source_href(lead.get("url") or "#"))}" target="_blank" rel="noreferrer">{esc(lead.get("title") or lead.get("lead_id"))}</a>'
            for lead in track.get("lead_refs", [])
            if isinstance(lead, dict) and app.source_href(lead.get("url") or "#") != "#"
        ) or "暂无外部追索线索"
        track_cards.append(
            f'''<article class="result compact-result"><div>
  <h3>{esc(track.get("title") or track.get("track_id"))}</h3>
  <div class="meta">{esc(track.get("period") or "时期未标注")} · {esc(track.get("track_role") or "研究轨道")} · {esc(track.get("status") or "未标注")}</div>
  <div class="snippet"><strong>研究问题：</strong>{esc(track.get("research_question"))}</div>
  <div class="snippet"><strong>证据边界：</strong>{esc(track.get("evidence_boundary"))}</div>
  <div class="snippet"><strong>已有候选：</strong>{candidate_links}</div>
  <div class="snippet"><strong>追索线索：</strong>{lead_links}</div>
  <div class="snippet"><strong>下一步：</strong>{esc(track.get("next_action"))}</div>
</div><div class="cite"><span class="pstatus warn">主证据仍开放</span></div></article>'''
        )
    track_body = "".join(track_cards) or '<div class="notice">本专题没有登记前置研究轨道。</div>'
    track_html = (
        '<div class="section-head"><h2>前置研究轨道</h2><span class="meta">元数据层，不新增事件门槛</span></div>'
        '<div class="notice">研究轨道只组织阶段、问题和来源入口，不复制正文，也不改变主证据闭环状态。</div>'
        f'<section class="result-list">{track_body}</section>'
    )
