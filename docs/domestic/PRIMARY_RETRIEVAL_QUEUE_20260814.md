...
这份队列把 `event_coverage.json` 中九个专题的 `missing_primary` 目标，与候选记录、官方访问审计和正式库页级元数据对齐。当前候选元数据和正式库均为 690 条记录。队列现为 `domestic_primary_retrieval_queue.v3`：对已关联正式库的候选保留页 ID、页标签和严格页标记，同时把已有的专题页级回链以 `event_link_pages` 叠加到对应专题，收口看板不会漏掉已核验的导航入口。它回答的是“下一步去哪里取得或核验原件”，不是“这些候选已经证明了事件”。
