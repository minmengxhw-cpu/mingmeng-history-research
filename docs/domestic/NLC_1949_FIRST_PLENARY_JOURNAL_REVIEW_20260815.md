...
| PDF 页 | 发现 | 当前状态 |
|---:|---|---|
| 1 | 会刊封面，标明 1949 年、第一期至第十二期 | `human_verified`（仅页级身份） |
| 17 | 第一届全体会议开幕式程序，日期 1949-09-21 | `human_verified`（仅页级身份与日期） |
| 30 | 主席团名单；可见张澜、沈钧儒、章伯钧、张东荪、史良、沙千里等人物 | `human_verified`（名册页定位；逐人校勘未完成） |
| 31–32 | 第一届全体会议议事规则 | `human_verified`（仅页级身份） |
| 220 | 第一届全体会议宣言（草案），日期 1949-09-30 | `human_verified`（仅页级身份与日期；2026-08-17） |
...
因此本轮把 8 个选定页以空正文记录写入 SQLite，并写入页级 provenance；其中 PDF 1、17、30、31、32、220 页已在独立页级身份审查后标为 `human_verified`/`citation_ready=1`，但只允许页级身份与范围引用；PDF 242、243 仍为 `review_only`/`citation_ready=0`。8 页均不写入正文、不生成 OCR 正文，专题状态仍不改成 `primary_evidence_closed`。正式库记录为 document `1600`、pages `20932–20939`。

页级审查记录见 [`nlc_1949_conference_journal_page_identity_review_20260815.json`](../../work/domestic/nlc_1949_conference_journal_page_identity_review_20260815.json)。
...
下一步按“原件身份 → 页级 provenance → 人工复核 → 正式库/FTS → 专题门禁”的顺序处理选定页；第 30 页仍需单独完成逐人姓名和身份对位，第 17 页仍需决定是否扩展到连续程序页。任何 OCR 仍只能作为检索草稿，不能自动升级为可引用正文。
