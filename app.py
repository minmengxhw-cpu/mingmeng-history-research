...
LOCAL_METADATA_DISCOVERY_REPORT_PATH = WORK_ROOT / "domestic" / "local_desktop_metadata_inventory_20260831" / "REPORT.json"
LOCAL_METADATA_REVIEW_QUEUE_REPORT_PATH = WORK_ROOT / "domestic" / "local_desktop_metadata_inventory_20260831" / "REVIEW_QUEUE_REPORT.json"
LOCAL_OCR_DISPOSITION_REPORT_PATH = WORK_ROOT / "domestic" / "local_desktop_metadata_inventory_20260831" / "OCR_DISPOSITION_REPORT.json"
LOCAL_DEDUPE_PLAN_REPORT_PATH = WORK_ROOT / "domestic" / "local_desktop_metadata_inventory_20260831" / "DEDUPE_PLAN_REPORT.json"
LOCAL_FORMAL_HASH_RECONCILIATION_REPORT_PATH = WORK_ROOT / "domestic" / "local_formal_hash_reconciliation_current" / "REPORT.json"
...
RESEARCH_TRACKS_PATH = DATA_ROOT / "domestic" / "research_tracks.json"
...
ACADEMIC_SOURCE_REUSE_MAP_PATH = DATA_ROOT / "domestic" / "academic_formal_reuse_map.json"
ACADEMIC_GAR639_PAGE_IDENTITY_CANDIDATE_PATH = DATA_ROOT / "domestic" / "academic_gar639_page_identity_candidate.json"
SIBLING_COLLECTION_INTAKE_QUEUE_PATH = DATA_ROOT / "domestic" / "sibling_collection_intake_queue.json"
...
CONTENT_TIER_AUDIT_REPORT_GLOB = "domestic/content_tier_audit_*/REPORT.json"
...
CITATION_FRAGMENT_LEDGER_PATH = DATA_ROOT / "domestic" / "citation_fragments.jsonl"
CITATION_FRAGMENT_MANIFEST_PATH = DATA_ROOT / "domestic" / "citation_fragments_manifest.json"
...
def _load_content_tier_audit_report() -> dict[str, object]:
    """Load the newest valid local content-layer audit without page bodies.

    Audits are intentionally written to dated work directories.  The quality
    page must not keep showing an older snapshot merely because the canonical
    path has an older date in its name, so choose by the report timestamp and
    use file mtime only as a deterministic fallback.
    """
    candidates = {CONTENT_TIER_AUDIT_REPORT_PATH}
    candidates.update(WORK_ROOT.glob(CONTENT_TIER_AUDIT_REPORT_GLOB))
    valid: list[tuple[str, float, str, dict[str, object]]] = []
    for path in candidates:
        ...
        if not isinstance(payload, dict):
            continue
        if payload.get("schema_version") != "domestic_content_tier_audit.v1":
            continue
        generated_at = str(payload.get("generated_at") or "")
        valid.append((generated_at, mtime, str(path), payload))
    if not valid:
        return {}
...
<div class="snippet">{h(row.get("use") or "")}</div></div></article>'''
    ...
    generated_at = str(report.get("generated_at") or "日期未标注")
...
<div class="notice"><span class="pstatus {status_class}">{h(status)}</span> 审计快照：{h(generated_at)}。该报告只读元数据，不读取正文、不写正式 SQLite、不改变 citation-ready、不删除文件。层级是平台工作角色，不是历史真实性等级；未映射来源类型必须先补角色。</div>
<section class="result-list">{cards}</section>'''


def _load_local_metadata_operations_summary() -> dict[str, object]:
    """Load safe scalar counts from the private local discovery reports.

    The reports contain local paths and are intentionally kept out of public
    output.  This loader copies only non-negative aggregate counts and boolean
    safety flags, so the quality page can show the operational state without
    exposing a filesystem layout or reading any source body.
    """
    ...
    def count(payload: dict[str, object], key: str) -> int:
    ...
    def amount(payload: dict[str, object], key: str) -> float:
        try:
    ...
    discovery = read_report(LOCAL_METADATA_DISCOVERY_REPORT_PATH)
    queue = read_report(LOCAL_METADATA_REVIEW_QUEUE_REPORT_PATH)
    ocr = read_report(LOCAL_OCR_DISPOSITION_REPORT_PATH)
    dedupe = read_report(LOCAL_DEDUPE_PLAN_REPORT_PATH)
    if not any((discovery, queue, ocr, dedupe)):
        return {}
    return {
        "inventory_file_count": count(discovery, "file_count"),
        "review_first_count": count(queue, "review_first_count"),
        "p0_exact_filename_match_count": count(queue, "p0_exact_filename_match_count"),
        "text_layer_check_count": count(
            ocr.get("disposition_counts") if isinstance(ocr.get("disposition_counts"), dict) else {},
            "TEXT_LAYER_CHECK_BEFORE_OCR",
        ),
        "ocr_provenance_hold_count": count(
            ocr.get("disposition_counts") if isinstance(ocr.get("disposition_counts"), dict) else {},
            "HOLD_OCR_UNTIL_PROVENANCE_REVIEW",
        ),
        "duplicate_group_count": count(dedupe, "group_count"),
        "duplicate_member_file_count": count(dedupe, "member_file_count"),
        "duplicate_recoverable_mib": amount(dedupe, "mib_recoverable_if_one_copy_retained"),
        "body_read": False,
        "ocr_performed": False,
...
def _local_metadata_operations_html(summary: dict[str, object]) -> str:
    """Render private discovery operations as aggregate counts only."""

    if not summary:
        return '''<div class="section-head"><h2><svg class="ico"><use href="#i-archive"/></svg>本机资料发现层</h2></div>
<div class="notice">本机资料发现层尚未挂载到当前 checkout；这里不会扫描、读取或上传本机文件。</div>'''
    return f'''<div class="section-head"><h2><svg class="ico"><use href="#i-archive"/></svg>本机资料发现层</h2><span class="meta">元数据只读</span></div>
<section class="stats">
  <div class="stat"><strong>{h(summary.get("inventory_file_count", 0))}</strong><span>发现层文件</span></div>
  <div class="stat"><strong>{h(summary.get("review_first_count", 0))}</strong><span>优先待审</span></div>
  <div class="stat"><strong>{h(summary.get("text_layer_check_count", 0))}</strong><span>先查文本层</span></div>
  <div class="stat"><strong>{h(summary.get("ocr_provenance_hold_count", 0))}</strong><span>来源准入后 OCR</span></div>
  <div class="stat"><strong>{h(summary.get("duplicate_group_count", 0))}</strong><span>重复组</span></div>
  <div class="stat"><strong>{h(summary.get("duplicate_recoverable_mib", 0))}</strong><span>理论可释放 MiB</span></div>
</section>
<div class="notice"><span class="pstatus ok">只读</span> 该层只显示聚合计数；文件名信号不等于史料价值，重复组不自动删除，电子文件先检查文本层，图片先补来源与页身份。正文读取、OCR、正式入库、复制/移动和删除均未发生。</div>'''


def _load_local_formal_hash_reconciliation_summary() -> dict[str, object]:
    """Load only aggregate results from the private hash reconciliation report."""

    path = LOCAL_FORMAL_HASH_RECONCILIATION_REPORT_PATH
    if not path.is_file():
        return {}
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    ...
    return {
        "inventory_file_count": count("inventory_file_count"),
        "formal_existing_file_count": count("formal_existing_file_count"),
        "local_hashed_file_count": count("local_hashed_file_count"),
        "formal_hashed_file_count": count("formal_hashed_file_count"),
        "exact_formal_source_path_count": count("exact_formal_source_path_count"),
        "exact_formal_source_hash_local_file_count": count("exact_formal_source_hash_local_file_count"),
        "unmatched_local_file_count": count("unmatched_local_file_count"),
        "body_read": False,
        "formal_db_written": False,
    }


def _local_formal_hash_reconciliation_html(summary: dict[str, object]) -> str:
    """Render the private hash reconciliation as safe aggregate counts only."""

    if not summary:
        return '''<div class="section-head"><h2><svg class="ico"><use href="#i-shield"/></svg>本机—正式库哈希对账</h2></div>
<div class="notice">本机—正式库哈希对账报告尚未挂载；这里不会扫描、读取或上传本机文件。</div>'''
    exact_path = int(summary.get("exact_formal_source_path_count", 0) or 0)
    exact_hash = int(summary.get("exact_formal_source_hash_local_file_count", 0) or 0)
    return f'''<div class="section-head"><h2><svg class="ico"><use href="#i-shield"/></svg>本机—正式库哈希对账</h2><span class="meta">字节哈希只读</span></div>
<section class="stats">
  <div class="stat"><strong>{h(summary.get("local_hashed_file_count", 0))}</strong><span>本机已哈希</span></div>
  <div class="stat"><strong>{h(summary.get("formal_hashed_file_count", 0))}</strong><span>正式库已哈希</span></div>
  <div class="stat"><strong>{h(exact_path + exact_hash)}</strong><span>精确匹配</span></div>
  <div class="stat"><strong>{h(summary.get("unmatched_local_file_count", 0))}</strong><span>未直接匹配</span></div>
</section>
<div class="notice"><span class="pstatus ok">仅身份对账</span> 精确路径匹配 {h(exact_path)}、精确字节哈希匹配 {h(exact_hash)}。哈希只说明文件字节身份，不证明来源、权利或史料价值；正文读取、OCR、正式入库、复制/移动和删除均未发生。</div>'''
...
NAV_GROUPS = [
    ...
    ("workbench", "i-edit", "研究工作台", [
        ...
        ("/domestic/citations/fragments", "国内片段证据台账"),
        ("/align", "多源对位"),
        ("/cards", "证据卡片库"),
        ("/tasks", "校订任务"),
        ("/quality", "质量检查"),
...
def _load_citation_fragment_rows() -> list[dict[str, object]]:
    """Load the additive fragment ledger; never infer page-level citation status."""
...
def _citation_fragments_for_page(page_id: int) -> list[dict[str, object]]:
    return [
        row
        for row in _load_citation_fragment_rows()
        if int(row.get("main_db_page_id") or 0) == int(page_id)
    ]
...
def domestic_fragment_panel(page_id: int) -> str:
    ...
    cards: list[str] = []
    for row in rows:
        text = f"{row.get('text') or ''}{row.get('display_suffix') or ''}"
        cards.append(
            f"""
<article class="result compact-result">
  ...
  <div class="cite"><a href="/domestic/citations/fragments?q={quote(str(row.get('target_id') or ''))}">片段台账</a><br><a href="{h(source_href(row.get('page_url') or '#'))}" target="_blank" rel="noreferrer">原始来源</a></div>
</article>"""
        )
    return f"""
...
def domestic_fragment_ledger_page(qs: dict[str, list[str]]) -> bytes:
    """Display the unified ledger of short, manually verified fragments."""
    ...
    try:
        loaded = json.loads(CITATION_FRAGMENT_MANIFEST_PATH.read_text(encoding="utf-8"))
        if isinstance(loaded, dict):
            manifest = loaded
    except (OSError, json.JSONDecodeError):
        manifest = {}
    ...
    for row in filtered:
        ...
        source_url = source_href(row.get("page_url") or "#")
        cards.append(
            f"""
<article class="result">
  <div>
    ...
    <div class="snippet"><strong>边界：</strong>{h(row.get('boundary_status') or '未登记')}；{h(row.get('promotion_scope') or '')}</div>
  </div>
  <div class="cite"><a href="/cite/{h(page_id)}">页级引用门禁</a><br><a href="{h(source_url)}" target="_blank" rel="noreferrer">原始来源</a><br><span class="meta">SHA {h(str(row.get('source_sha256') or '')[:16])}…</span></div>
</article>"""
        )
    search_value = h(query)
...
<section class="doc-head">
  <div><h1>国内片段证据台账</h1><div class="meta">把已人工核验的短片段接入国内—海外统一研究路径；片段级可引用不等于整页 citation-ready。</div></div>
  <div class="doc-tools"><a class="button" href="/domestic/workbench">国内研究平台</a><a class="button secondary" href="/domestic/sourcebook/1946-pcc">1946 政協文獻</a></div>
</section>
<section class="stats"><div class="stat"><strong>{h(manifest.get('fragment_count') or len(rows))}</strong><span>已登记片段</span></div><div class="stat"><strong>{h(manifest.get('fragment_citation_ready_count') or 0)}</strong><span>片段级可引用</span></div><div class="stat"><strong>{h(manifest.get('page_citation_ready_count') or 0)}</strong><span>整页可引用</span></div><div class="stat"><strong>{h(manifest.get('formal_db_written_count') or 0)}</strong><span>正式库写入</span></div></section>
<div class="notice"><strong>证据纪律：</strong>这些记录只展示经过视觉交叉核对的短片段，并保留来源文件 SHA256、PDF/印刷页和边界说明。机器 OCR 仅用于交叉检索；整页正文仍保持 <code>body_read=false</code>、<code>page_citation_ready=false</code>，不会因为台账存在而升级。</div>
<form class="search" method="get" action="/domestic/citations/fragments" role="search"><input type="search" name="q" value="{search_value}" placeholder="搜索人物、主题、片段或页码"><button type="submit">筛选片段</button></form>
<section class="result-list">{''.join(cards) or '<div class="notice">没有匹配的片段记录。</div>'}</section>
"""
    return layout("国内片段证据台账", body, query=query, active_path="/domestic/citations/fragments")
...
<section class="doc-head" style="margin-top:20px;background:var(--panel-warm);border-left:4px solid var(--accent);">
  ...
  <div class="doc-tools"><a class="button" href="/research/domestic-1946-pcc/packet">研究包</a><a class="button" href="/domestic/citations/fragments">片段证据台账</a><a class="button secondary" href="/research/gaps">证据缺口</a></div>
</section>
"""
...
def _domestic_core_documents_sql(
    ...
    sql = f"""
        SELECT
            ...
            MIN(CASE WHEN {DOMESTIC_STRICT_CITATION_SQL} THEN p.id END) AS first_citation_page_id,
...
"""
    if not rows:
        ...
        for row in rows:
            ...
            citation_link = (
                f'<a href="/cite/{h(row["first_citation_page_id"])}">首张引用卡</a>'
                if cite > 0 and row["first_citation_page_id"]
                else ""
            )
            body += f"""
<article class="result">
  ...
  <div class="cite"><a href="{h(href)}">阅读文献</a>{f'<br>{citation_link}' if citation_link else ''}<br><a href="{h(source_href)}" target="_blank" rel="noreferrer">原始来源</a></div>
</article>"""
    body += "</section>"
...
<article class="result">
  ...
  <div class="cite"><a href="{h(href)}">阅读</a>{f'<br>{citation_link}' if citation_link else ''}</div>
</article>""")
...
def domestic_quality_page() -> bytes:
    ...
    local_metadata_section = _local_metadata_operations_html(
        _load_local_metadata_operations_summary()
    )
    local_formal_hash_section = _local_formal_hash_reconciliation_html(
        _load_local_formal_hash_reconciliation_summary()
    )
...
def domestic_academic_page() -> bytes:
    ...
    academic_source_reuse = _load_academic_source_reuse_map()
...
<div class="notice">该队列只从版本化学术元数据中生成，优先安排 S/A 记录；它不读取正文、不包含本地路径、不写正式库。P0/P1 是下一轮获取稳定全文和页码的工作入口，完成页级取证后仍需 SHA-256、版本关系和复核，citation-ready 不会由队列自动改变。<br><a href="/domestic/search?scope=research&amp;availability=fulltext">查看稳定全文候选</a> · <a href="/domestic/search?scope=research&amp;availability=candidate">查看全文候选</a></div>'''
        ...
        for record in sorted(
            ...
            reuse_record = academic_source_reuse.get(external_id) or {}
            reuse_doc_key = str(reuse_record.get("existing_doc_key") or "").strip()
            reuse_html = ""
            if reuse_doc_key and reuse_record.get("reuse_status") == "REUSE_EXISTING_PAGE_OCR_NO_DUPLICATE_INGEST":
                reuse_href = f"/doc/{quote(reuse_doc_key, safe='')}"
                reuse_html = (
                    f'<div class="meta">已有同 SHA 页级 OCR：{h(reuse_record.get("expected_page_count", 0))} 页 · '
                    f'<a href="{h(reuse_href)}">打开已有页级 OCR 入口</a> · 不重复 OCR</div>'
                )
            if external_id == "GAR-639C5E94AE":
                identity_candidate = _load_academic_gar639_page_identity_candidate()
                if identity_candidate.get("status") == "CANDIDATE_NOT_REGISTERED":
                    reuse_html += (
                        '<div class="meta">书内页码候选：PDF 页 − 30（11 个分布式锚点）· '
                        '严格 printed_page 未登记（0/622）· 不改变 citation_ready</div>'
                    )
...
def _academic_source_quality_warning(record: dict[str, object]) -> str:
    """Show a visible warning for a source identity reconciliation hold.

    The warning is driven by tracked metadata only.  It never reads a source
    body and never turns an alias page into a formal academic result.
    """
    audit_record = record
    if str(record.get("source_quality_audit") or "") != "WRONG_PAGE_ALIAS_HOLD":
        external_id = str(record.get("external_id") or "").strip()
        if external_id:
            audit_record = next(
                (
                    candidate
                    for candidate in _load_academic_metadata_index()
                    if str(candidate.get("external_id") or "").strip() == external_id
                ),
                record,
            )
    if str(audit_record.get("source_quality_audit") or "") != "WRONG_PAGE_ALIAS_HOLD":
        return ""
    matched_id = str(audit_record.get("matched_record_id") or "")
    observed_title = str(audit_record.get("observed_page_title") or "")
    details = f"实际页面标题：{h(observed_title)}" if observed_title else "实际页面身份待核"
    matched = f"（对应记录 {h(matched_id)}）" if matched_id else ""
    return (
        f'<div class="notice"><strong>来源身份警告：</strong>当前公开入口被对账为错误页面别名，'
        f'{details}{matched}；它不能作为本条目标资料的完整正文，也不能据此正式导入或引用。</div>'
    )
...
def domestic_staging_search_page(query: dict[str, list[str]] | None = None) -> bytes:
    ...
    if scope == "documents":
        ...
        academic_quality_by_id = {
            str(record.get("external_id") or ""): record
            for record in _load_academic_metadata_index()
            if isinstance(record, dict)
            and str(record.get("source_quality_audit") or "") == "WRONG_PAGE_ALIAS_HOLD"
        }
        result_html = "".join(
            ...
            <div class="snippet">{h(row["institution"] or "机构未标注")} · 机构字段信号：{h(_academic_institution_signal_label(_academic_institution_signal_value(row["institution"]))) } · 出版/发表 {h(row["publication_date"] or "未标注")} · {h(row["fulltext_status"])} · citation_ready={h(row["citation_ready"])} · human_verified={h(row["human_verified"])}</div>{_academic_source_quality_warning({"external_id": str(row["external_id"]), **academic_quality_by_id.get(str(row["external_id"]), {})})}</div><div class="cite"><a href="{h(source_href(row["source_url"] or "#"))}">来源入口</a>{_academic_result_links(str(row["external_id"]), formal_academic, related_formal)}</div></article>'''
            for row in rows
        )
    else:
        claim_results = []
...
def _load_domestic_research_tracks(event_id: str = "") -> list[dict[str, object]]:
    """读取挂在固定专题下的研究轨道元数据，不新增事件或读取正文。"""
    if not RESEARCH_TRACKS_PATH.is_file():
        return []
    try:
        payload = json.loads(RESEARCH_TRACKS_PATH.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return []
    ...
    return [
        ...
        and (not event_id or str(track.get("event_id") or "") == event_id)
    ]
...
def research_gaps_page() -> bytes:
    ...
    intake_target_count = 0
    try:
        target_payload = json.loads(
            AUTHORIZED_ORIGINAL_INTAKE_TARGETS_PATH.read_text(encoding="utf-8")
        )
        target_rows = target_payload.get("targets", []) if isinstance(target_payload, dict) else []
        if isinstance(target_rows, list):
            intake_target_count = sum(isinstance(row, dict) for row in target_rows)
    except (OSError, json.JSONDecodeError):
        intake_target_count = 0

    intake_report: dict[str, object] = {}
    try:
        loaded_intake = json.loads(
            AUTHORIZED_ORIGINAL_INTAKE_REPORT_PATH.read_text(encoding="utf-8")
        )
        if isinstance(loaded_intake, dict):
            intake_report = loaded_intake
    except (OSError, json.JSONDecodeError):
        intake_report = {}
    try:
        intake_target_count = int(
            intake_report.get("target_count") or intake_target_count
        )
    except (TypeError, ValueError):
        pass
    intake_status = _authorized_original_intake_report_status(
        intake_report, intake_target_count
    )
    intake_status_label = {
        "NOT_RUN": "尚未运行接收检查",
    ...
    }.get(intake_status, intake_status)
    if intake_report:
        intake_note = (
            f'<div class="notice"><strong>授权原件接收前置状态：</strong>'
            f'目标 {h(intake_target_count)} · '
            f'incoming 文件 {h(intake_report.get("incoming_file_count") or 0)} · '
            f'映射 {h(intake_report.get("mapping_count") or 0)} · '
            f'<span class="pstatus warn">{h(intake_status_label)}</span>。'
            f' <a href="/domestic/intake">打开接收门禁</a>。'
            '该摘要只表示文件接收前置检查，不等于正文已读或一手证据闭环。</div>'
        )
    else:
        intake_note = (
            f'<div class="notice"><strong>授权原件接收前置状态：</strong>'
            f'目标 {h(intake_target_count)} · '
            '<span class="pstatus warn">尚未生成本机接收报告</span>。'
            f' <a href="/domestic/intake">打开接收门禁</a>。'
            '跟踪清单中的目标数量不等于已经取得原件；请在有本机资料的 checkout 中运行接收检查。'
            '</div>'
        )
...
def _load_sibling_collection_intake_queue() -> dict[str, object]:
    """读取 sibling 采集包的安全元数据待准入队列，不读取正文。"""
    if not SIBLING_COLLECTION_INTAKE_QUEUE_PATH.is_file():
        return {}
    try:
        payload = json.loads(SIBLING_COLLECTION_INTAKE_QUEUE_PATH.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return {}
    if not isinstance(payload, dict) or payload.get("schema_version") != "domestic_sibling_collection_intake_queue.v1":
        return {}
    safe_flags = (
        "body_read",
        "ocr_performed",
    ...
    if any(payload.get(key) is not False for key in safe_flags):
        return {}
    records = payload.get("records")
    if not isinstance(records, list) or not all(isinstance(record, dict) for record in records):
        return {}
...
def _load_academic_source_reuse_map() -> dict[str, dict[str, object]]:
    """读取已有页级 OCR 的安全复用映射，不读取正文或本地路径。"""
    if not ACADEMIC_SOURCE_REUSE_MAP_PATH.is_file():
        return {}
    try:
        payload = json.loads(ACADEMIC_SOURCE_REUSE_MAP_PATH.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return {}
    if not isinstance(payload, dict) or payload.get("schema_version") != "domestic_academic_source_reuse_map.v1":
        return {}
    if any(payload.get(key) is not False for key in ("body_read", "formal_db_written", "local_paths_included", "auto_delete")):
        return {}
    records = payload.get("records")
    if not isinstance(records, list):
        return {}
    ...
    for record in records:
        if not isinstance(record, dict):
            continue
        external_id = str(record.get("external_id") or "").strip()
        if external_id and record.get("existing_doc_key"):
            result[external_id] = record
    return result


def _load_academic_gar639_page_identity_candidate() -> dict[str, object]:
    """读取 GAR-639 页码候选状态，不读取正文或本地路径。"""
    if not ACADEMIC_GAR639_PAGE_IDENTITY_CANDIDATE_PATH.is_file():
        return {}
    try:
        payload = json.loads(ACADEMIC_GAR639_PAGE_IDENTITY_CANDIDATE_PATH.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return {}
    if not isinstance(payload, dict):
        return {}
    if payload.get("schema_version") != "domestic_academic_gar639_page_identity_candidate.v1":
        return {}
    if any(payload.get(key) is not False for key in ("body_text_included", "formal_db_written", "local_paths_included", "auto_delete")):
        return {}
    candidate = payload.get("candidate_mapping")
...
def _research_academic_matches(
    ...
    for source in source_rows:
        if _academic_record_is_gap_note(source):
            continue
        if source.get("academic_crosswalk_eligible", True) is False:
            continue
        try:
...
def _research_topic_rows() -> list[dict[str, object]]:
    ...
    with conn() as c:
        for item in coverage:
            ...
            research_tracks = _load_domestic_research_tracks(event_id)
            result.append({
                "item": item,
...
def domestic_workbench_page() -> bytes:
    ...
    sibling_intake = _load_sibling_collection_intake_queue()
    sibling_intake_counts = sibling_intake.get("disposition_counts") or {}
    sibling_intake_count = int(sibling_intake.get("queue_record_count") or 0)
    nav_ready = 0
    bounded_research = 0
    primary_closed = 0
    research_ready_count = 0
...
<div class="cite">
  ...
  <a href="/domestic/acquisition?event={quote(event_id)}">调档</a>
    ...
    sibling_intake_note = (
        f'<div class="notice"><strong>Sibling 采集包待准入：</strong>{h(sibling_intake_count)} 条未匹配元数据；'
        f'官方/机构候选 {h(sibling_intake_counts.get("PROMOTE_METADATA_REVIEW") or 0)} 条，'
        f'学术身份待补 {h(sibling_intake_counts.get("PROMOTE_ACADEMIC_METADATA_REVIEW") or 0)} 条。'
        '这只是来源身份和版本对账队列，不等于正文已读或正式库已接收；'
        '<a href="/domestic/review#sibling-intake">查看准入复核</a>。</div>'
        if sibling_intake
        else ""
    )
...
def _authorized_original_intake_report_status(
    report: dict[str, object], target_count: int
) -> str:
    """Return an honest intake status even for legacy reports without ``status``.

    The intake generator historically emitted ``status_counts`` but no top-level
    status.  When every configured target is explicitly waiting for an original,
    that is a completed check with an open input gate, not an unrun check.
    """
    explicit = str(report.get("status") or "").strip()
    if explicit:
        return explicit
...
<article class="result compact-result"><div>
  ...
  <div class="snippet">缺少或待复核字段：{h(missing_label)}</div>
    ...
    report_status = _authorized_original_intake_report_status(report, len(targets))
...
<section class="doc-tools" style="margin-top:20px;justify-content:center;">
  <a class="button" href="/domestic/workbench">国内研究平台</a>
  <a class="button" href="/research/questions">研究问题入口</a>
  <a class="button" href="/research/tracks">研究轨道</a>
...
def research_tracks_page() -> bytes:
    """List phase-oriented research tracks without creating new event gates."""
    topics = _research_topic_rows()
    cards: list[str] = []
    ...
    for topic in topics:
        item = topic.get("item") or {}
        event_id = str(item.get("event_id") or "")
        for track in topic.get("research_tracks", []):
            if not isinstance(track, dict):
                continue
            event_ids.add(event_id)
            candidate_count = len(track.get("candidate_ids") or [])
            lead_count = len(track.get("lead_refs") or [])
            cards.append(f"""
<article class="result compact-result">
  <div>
    <h2>{h(track.get("title") or track.get("track_id"))}</h2>
    <div class="meta">{h(track.get("period") or "时期未标注")} · 所属专题：<a href="/research/{quote(event_id, safe='')}">{h(item.get("event_name") or event_id)}</a> · 候选 {h(candidate_count)} 条 · 追索线索 {h(lead_count)} 条</div>
    <div class="tagline"><span class="pstatus warn">{h(track.get("status") or "未标注")}</span><span class="tag">主证据仍开放</span><span class="tag">正文不复制</span></div>
    <div class="snippet"><strong>研究问题：</strong>{h(track.get("research_question"))}</div>
    <div class="snippet"><strong>边界：</strong>{h(track.get("evidence_boundary"))}</div>
  </div>
  <div class="cite"><a href="/research/{quote(event_id, safe='')}">进入所属专题</a><br><a href="/research/{quote(event_id, safe='')}/packet">打开研究包</a></div>
</article>""")
    body = breadcrumb_html([("/", "首页"), ("/research", "多源专题研究"), (None, "研究轨道")]) + f"""
<section class="hero hero-compact">
  <div class="hero-eyebrow">PHASE-ORIENTED RESEARCH TRACKS</div>
  <h1>研究轨道</h1>
  <p class="hero-sub">按历史阶段和研究问题组织材料；研究轨道是元数据导航，不新增事件门槛，也不把候选记录升级为一手证据。</p>
  <div class="hero-chips"><span><b>{h(len(cards))}</b> 条轨道</span><span><b>{h(len(event_ids))}</b> 个所属专题</span></div>
</section>
<div class="notice">轨道页只提供阶段、候选、线索和证据边界。正式引用仍需回到原文页、页级 provenance、SHA256 和 citation gate；开放 P0 原件缺口不会因轨道出现而自动关闭。</div>
<section class="result-list">{"".join(cards) or '<div class="notice">当前没有登记研究轨道。</div>'}</section>
<section class="doc-tools" style="margin-top:20px;justify-content:center;"><a class="button" href="/research">专题索引</a><a class="button" href="/research/questions">研究问题入口</a><a class="button secondary" href="/domestic/workbench">国内研究平台</a></section>
"""
    return layout("研究轨道", body, active_path="/research")
...
<article class="result compact-result"><div>
  ...
  <div class="snippet">{h(row['evidence_note'] or row['uncertainty_note'])}</div>
    ...
    domestic_by_id = {str(row["candidate_id"]): row for row in topic["domestic_rows"]}
    research_track_cards: list[str] = []
    for track in topic.get("research_tracks", []):
        if not isinstance(track, dict):
            continue
        candidate_links: list[str] = []
        for candidate_id in track.get("candidate_ids") or []:
            candidate_key = str(candidate_id)
            row = domestic_by_id.get(candidate_key)
            label = str(row["title"] if row else candidate_key)
            level = str(row["authenticity_level_proposed"] if row else "未分级")
            status = str(row["review_status"] if row else "未回接")
            candidate_links.append(
                f'<a href="/domestic/candidate/{quote(candidate_key, safe="")}">{h(label)}</a>'
                f' <span class="meta">{h(level)} / {h(status)}</span>'
            )
        lead_links: list[str] = []
        for lead in track.get("lead_refs") or []:
            if not isinstance(lead, dict):
                continue
            lead_url = source_href(lead.get("url") or "#")
            link = (
                f'<a href="{h(lead_url)}" target="_blank" rel="noreferrer">{h(lead.get("title") or lead.get("lead_id"))}</a>'
                if lead_url != "#"
                else h(lead.get("title") or lead.get("lead_id"))
            )
            lead_links.append(
                f'{link} <span class="meta">{h(lead.get("evidence_level") or "未分级")} · {h(lead.get("role") or "追索线索")}</span>'
            )
        track_candidates_html = " · ".join(candidate_links) or '<span class="meta">尚无候选回链</span>'
        track_leads_html = " · ".join(lead_links) or '<span class="meta">尚无外部追索线索</span>'
        research_track_cards.append(f"""
<article class="result compact-result" style="border-left:4px solid var(--accent);background:var(--panel-warm);">
  <div>
    <h3>{h(track.get("title") or track.get("track_id"))}</h3>
    <div class="meta">{h(track.get("period") or "时期未标注")} · {h(track.get("track_role") or "研究轨道")} · {h(track.get("status") or "未标注")}</div>
    <div class="snippet"><strong>研究问题：</strong>{h(track.get("research_question"))}</div>
    <div class="snippet"><strong>证据边界：</strong>{h(track.get("evidence_boundary"))}</div>
    <div class="snippet"><strong>处理政策：</strong>{h(track.get("processing_policy"))}</div>
    <div class="snippet"><strong>已有候选：</strong>{track_candidates_html}</div>
    <div class="snippet"><strong>追索线索：</strong>{track_leads_html}</div>
    <div class="snippet"><strong>下一步：</strong>{h(track.get("next_action"))}</div>
  </div>
  <div class="cite"><span class="pstatus warn">主证据仍开放</span></div>
</article>""")
    track_cards_html = "".join(research_track_cards) or '<div class="notice">当前专题没有额外研究轨道。</div>'
    research_tracks_html = (
        '<div class="section-head"><h2><svg class="ico"><use href="#i-branch"/></svg>前置研究轨道</h2><span class="meta">附着于现有专题，不新增事件门槛</span></div>'
        '<div class="notice">研究轨道用于把同一专题中的阶段、问题和来源分层呈现；它不把候选记录升级为正式证据，也不关闭 1947 年两个 P0 原件缺口。</div>'
        f'<section class="result-list">{track_cards_html}</section>'
    )
...
def domestic_review_page() -> bytes:
    ...
    sibling_intake = _load_sibling_collection_intake_queue()
    sibling_counts = sibling_intake.get("disposition_counts") or {}
    sibling_records = sibling_intake.get("records") or []
    priority_order = {"PROMOTE_METADATA_REVIEW": 0, "PROMOTE_ACADEMIC_METADATA_REVIEW": 1}
    sibling_priority_records: list[dict[str, object]] = []
    sibling_all_priority_records: list[dict[str, object]] = []
    for disposition, limit in (
        ("PROMOTE_METADATA_REVIEW", 8),
        ...
        selected = sorted(
            (
                ...
                if isinstance(record, dict)
                and str(record.get("disposition") or "") == disposition
            ),
            key=lambda record: (
                priority_order.get(str(record.get("disposition") or ""), 9),
                str(record.get("external_id") or ""),
            ),
        )
        sibling_all_priority_records.extend(selected)
        sibling_priority_records.extend(selected[:limit])
    def render_sibling_intake_cards(records: list[dict[str, object]]) -> str:
        return "".join(
            f'''<article class="result compact-result"><div>
  <h3>{h(record.get("title_hint") or record.get("external_id") or "未命名元数据")}</h3>
  <div class="meta">{h(record.get("external_id") or "未登记")} · {h(record.get("source_class") or "未分类")} · {h(record.get("disposition") or "待核")}</div>
  <div class="snippet">{h(record.get("disposition_reason") or "需补来源身份和版本关系")} · SHA {h((str(record.get("file_sha256") or "")[:16] or "未登记"))}…</div>
</div><div class="cite">{("<a href=\"" + h(str(record.get("source_url") or "")) + "\" target=\"_blank\" rel=\"noreferrer\">公开入口</a>") if str(record.get("source_url") or "").startswith(("http://", "https://")) else "入口待补"}</div></article>'''
            for record in records
        )
    sibling_priority_cards = render_sibling_intake_cards(sibling_priority_records)
    sibling_displayed_keys = {id(record) for record in sibling_priority_records}
    sibling_remaining_records = [
        record for record in sibling_all_priority_records if id(record) not in sibling_displayed_keys
    ]
    sibling_remaining_cards = render_sibling_intake_cards(sibling_remaining_records)
    sibling_remaining_section = (
        f'''<details class="sibling-intake-more"><summary>展开其余优先候选（{h(len(sibling_remaining_records))} 条）</summary>
<section class="result-list">{sibling_remaining_cards}</section></details>'''
        if sibling_remaining_records
        else ""
    )
    sibling_display_meta = (
        f'优先候选 {h(len(sibling_all_priority_records))} 条 · 默认显示 {h(len(sibling_priority_records))} 条'
        + (f' · 另有 {h(len(sibling_remaining_records))} 条可展开' if sibling_remaining_records else "")
    )
    sibling_intake_section = (
        f'''<div id="sibling-intake" class="section-head"><h2><svg class="ico"><use href="#i-layer"/></svg>Sibling 采集包元数据待准入</h2><span class="meta">{h(sibling_intake.get("queue_record_count") or 0)} 条未匹配 · body_read=false · {sibling_display_meta}</span></div>
<div class="notice">官方/机构候选 {h(sibling_counts.get("PROMOTE_METADATA_REVIEW") or 0)} 条，学术身份待补 {h(sibling_counts.get("PROMOTE_ACADEMIC_METADATA_REVIEW") or 0)} 条；已与主线候选 URL 重合的记录已排除。这里只显示公开 URL 和元数据提示，不显示本地路径或正文；完成身份、版本、权限和去重对账后，才可进入正式候选或全文队列。</div>
<section class="result-list">{sibling_priority_cards or '<div class="notice">当前没有待准入的官方/学术元数据。</div>'}</section>{sibling_remaining_section}'''
        if sibling_intake
        else ""
    )
...
_FRAGMENT_TRADITIONAL_TO_SIMPLIFIED = str.maketrans({
    "協": "协", "會": "会", "議": "议", "國": "国", "內": "内",
    "與": "与", "為": "为", "爭": "争", "組": "组", "張": "张",
    "瀾": "澜", "劢": "劢", "劑": "剂",
})
...
def _fragment_search_key(value: object) -> str:
    text = str(value or "")
    return text.translate(_FRAGMENT_TRADITIONAL_TO_SIMPLIFIED).casefold()


def _domestic_fragment_search_hits(
    query: str,
    platform: str | None,
    year: str,
    person: str,
    ...
    if not query.strip() and platform != "domestic":
        return []
    rows = _load_citation_fragment_rows()
    query_key = _fragment_search_key(query) if query.strip() else ""
    person_key = _fragment_search_key(person) if person.strip() else ""
    ...
    for row in rows:
        ...
        haystack = _fragment_search_key(raw_haystack)
...
def _domestic_fragment_search_html(rows: list[dict[str, object]]) -> str:
    if not rows:
        return ""
    cards: list[str] = []
    for row in rows:
...
<article class="result compact-result"><div>
  ...
  <div class="snippet">{h(row.get('scope') or '')}</div>
    ...
    return (
        '<div class="section-head" style="margin-top:20px;"><h2>国内片段级证据命中</h2>'
        f'<span class="meta">{len(rows)} 条 · 不替代整页引用</span></div>'
        '<div class="notice">这些命中来自人工视觉核验的短片段；正式页级引用状态仍须打开引用门禁确认。</div>'
        f'<section class="result-list">{"".join(cards)}</section>'
    )
...
def search(query: str, platform: str | None = None, year: str = "", grade: str = "", person: str = "", cited: bool = False) -> bytes:
    with conn() as c:
        ...
        fragment_hits = _domestic_fragment_search_hits(query, platform, year, person)
        body = stats_html(c)
        ...
        body += _domestic_fragment_search_html(fragment_hits)
        if not rows and not fragment_hits:
...
def citation_page(page_id: int) -> bytes:
    ...
    if citation_gated_platform and not _domestic_citation_is_strict(row):
        if row["source_platform"] == "drnh":
            ...
            gate_label = "国内史料"
        fragment_panel = domestic_fragment_panel(page_id) if row["source_platform"] == "domestic" else ""
        body = f"""
<section class="doc-head">
  <div>
    {title_block(row["title"], None, "h1")}
...
"""
    ...
    fragment_panel = (
        domestic_fragment_panel(page_id)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _do_GET_inner(self) -> None:
        ...
        elif parsed.path == "/domestic/citations/fragments":
            payload = domestic_fragment_ledger_page(qs)
        ...
        elif parsed.path == "/research/tracks":
            payload = research_tracks_page()
