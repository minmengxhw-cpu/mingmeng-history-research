...
**建国后（1949 之后）历次政治运动相关内容不在前台展示**：人物 profile / 关键事件 / 阶段简介中涉及反右、文革、政治运动结局等敏感史实的描述已统一移除前台。这部分内容可作为档案翻译的内部历史背景资料处理，但不进入对外页面。

涉及人物的 profile 字段统一脱敏到 1949 年前后的学术身份与建国初期非政治性职务（如「《光明日报》总编辑」「北京市副市长」等），不展开建国后的具体政治结局。

阶段分期措辞采用中性表述（如「建国初期民盟领袖与文化界盟员」），避免使用「政治运动期」「受难与过渡」等敏感措辞。
