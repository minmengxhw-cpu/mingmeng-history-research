...
def test_app_loads_newest_valid_content_tier_audit(tmp_path, monkeypatch):
    work_root = tmp_path / "work"
    old_path = work_root / "domestic" / "content_tier_audit_current_20260822" / "REPORT.json"
    new_path = work_root / "domestic" / "content_tier_audit_continue_20260830" / "REPORT.json"
    invalid_path = work_root / "domestic" / "content_tier_audit_future" / "REPORT.json"
    for path in (old_path, new_path, invalid_path):
    ...
    invalid_path.write_text(
        json.dumps(
            {
                "schema_version": "not_a_content_tier_audit",
    ...
    monkeypatch.setattr(app, "CONTENT_TIER_AUDIT_REPORT_PATH", old_path)
    monkeypatch.setattr(app, "CONTENT_TIER_AUDIT_REPORT_GLOB", "domestic/content_tier_audit_*/REPORT.json")

    report = app._load_content_tier_audit_report()

    assert report["generated_at"] == "2026-08-30T00:00:00+00:00"
