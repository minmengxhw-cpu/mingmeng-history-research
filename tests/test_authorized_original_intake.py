...
def test_empty_incoming_keeps_both_p0_targets_waiting(tmp_path: Path):
    ...
    assert all(row["missing_fields"] == ["local_path"] for row in manifest)
...
def test_intake_report_status_promotes_single_state_and_marks_mixed_state():
    assert intake_report_status(Counter({"WAITING_FOR_LOCAL_ORIGINAL": 2})) == "WAITING_FOR_LOCAL_ORIGINAL"
    assert intake_report_status(Counter()) == "NOT_RUN"
    assert intake_report_status(Counter({"WAITING_FOR_LOCAL_ORIGINAL": 1, "HASHED_NEEDS_PAGE_COUNT": 1})) == "PARTIAL"
