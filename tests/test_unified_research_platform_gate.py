...
def test_unified_platform_gate_passes_without_claiming_content_closure():
    ...
    candidate_alignment = report["checks"]["candidate_alignment"]
    assert candidate_alignment["missing_from_db"] == []
    assert candidate_alignment["discovery_only_count"] == 1
    assert candidate_alignment["discovery_only_in_db"] == []
    ...
    assert report["checks"]["academic_layer"]["fulltext_priority_queue_records"] == 23
    assert report["checks"]["academic_layer"]["fulltext_priority_queue_classes"] == {
        "P0_STABLE_FULLTEXT": 5,
        "P1_FULLTEXT_CANDIDATE": 12,
        "P2_STABLE_CONTEXT": 1,
        "P3_CANDIDATE_CONTEXT": 5,
    }
    assert report["checks"]["academic_layer"]["errors"] == []
    assert report["checks"]["academic_acquisition_reconciliation"]["status"] == "PASS"
    assert report["checks"]["academic_acquisition_reconciliation"]["records"] == 1
    reuse = report["checks"]["academic_source_reuse"]
    assert reuse["status"] == "PASS"
    assert reuse["database_status"] == "VERIFIED"
    assert reuse["verified_reuse_count"] == 1
    assert reuse["formal_academic_source_rows_for_reuse_candidates"] == 0
    assert reuse["records"][0]["page_count"] == 622
    assert reuse["records"][0]["printed_page_registered_count"] == 0
    assert reuse["records"][0]["page_label_printed_count"] == 17
    assert reuse["records"][0]["page_label_printed_citation_ready_count"] == 17
    assert reuse["records"][0]["review_status_counts"] == {
        "human_verified": 24,
        "machine_verified": 563,
        "review_only": 26,
        "unreadable": 9,
    }
    gar639_identity = report["checks"]["academic_gar639_page_identity_candidate"]
    assert gar639_identity["status"] == "PASS"
    assert gar639_identity["anchor_count"] == 11
    assert gar639_identity["database"]["printed_page_registered_count"] == 0
    academic_page = app.domestic_academic_page().decode("utf-8")
    assert "书内页码候选：PDF 页 − 30" in academic_page
    assert "严格 printed_page 未登记（0/622）" in academic_page
    formal_coverage = report["checks"]["academic_formal_coverage"]
    assert formal_coverage["formal_queue_coverage"] == 16
    assert formal_coverage["reused_queue_ids"] == ["GAR-639C5E94AE"]
    assert formal_coverage["preferred_queue_records"] == 17
    assert formal_coverage["context_only_queue_records"] == 6
    assert formal_coverage["preferred_formal_queue_coverage"] == 16
    assert formal_coverage["effective_queue_coverage"] == 17
    assert formal_coverage["effective_missing_ids"] == []
    assert len(formal_coverage["context_only_unmapped_ids"]) == 6
    assert report["checks"]["retrieval_queue"]["formal_candidate_count"] == 693
...
def test_drnh_1947_context_candidates_remain_metadata_only_review_leads():
    ...
    expected = {
        "domestic:DRNH:002-080200-00541-008",
        "domestic:DRNH:002-080200-00536-014",
        "domestic:DRNH:002-080200-00537-008",
    }
    assert expected <= rows.keys()
    for candidate_id in expected:
        record = rows[candidate_id]
        assert record["repository_code"] == "DRNH"
        assert record["authenticity_level_proposed"] == "L2"
        assert record["review_status"] == "needs_human_review"
        assert record["evidence_type"] == "catalogue"
        assert record["check_outcome"] == "needs_info"
        assert record["source_url"].startswith("https://ahonline.drnh.gov.tw/")
        assert "本地文件" in record["uncertainty_note"]
...
def test_academic_gate_uses_tracked_snapshot_without_staging_report(tmp_path, monkeypatch):
    ...
    assert result["fulltext_priority_queue_records"] == 23
...
def test_academic_search_surfaces_wrong_page_alias_warning():
    body = app.domestic_metadata_academic_search_page("对抗战建国协进会").decode("utf-8")
    assert "来源身份警告" in body
    assert "党派史话｜民盟历史知识两则" in body
    assert "不能作为本条目标资料的完整正文" in body
...
def test_domestic_workbench_is_the_research_front_door():
    ...
    assert "已有同 SHA 页级 OCR：622 页" in academic
    assert "/doc/domestic-page%2FSRC-257bb7be70" in academic
...
def test_authorized_original_intake_page_derives_waiting_status_from_legacy_report(tmp_path, monkeypatch):
    targets = json.loads(
        Path(app.AUTHORIZED_ORIGINAL_INTAKE_TARGETS_PATH).read_text(encoding="utf-8")
    )["targets"]
    report_path = tmp_path / "REPORT.json"
    manifest_path = tmp_path / "INTAKE_MANIFEST.jsonl"
    report_path.write_text(
        json.dumps(
            {
                "generated_at": "2026-08-30T00:00:00+08:00",
                "status_counts": {"WAITING_FOR_LOCAL_ORIGINAL": len(targets)},
    ...
    monkeypatch.setattr(app, "AUTHORIZED_ORIGINAL_INTAKE_REPORT_PATH", report_path)
    monkeypatch.setattr(app, "AUTHORIZED_ORIGINAL_INTAKE_MANIFEST_PATH", manifest_path)
    previous = getattr(app._request, "public_mode", False)
    app._request.public_mode = False
    try:
        body = app.domestic_authorized_intake_page().decode("utf-8")
    ...
    assert "WAITING_FOR_LOCAL_ORIGINAL" in body
    assert "尚未运行接收检查" not in body
