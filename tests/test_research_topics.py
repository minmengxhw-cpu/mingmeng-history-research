...
def test_research_gap_dashboard_smoke(live_server, db_missing_reason):
    ...
    assert "下一步" in body
    assert "授权原件接收前置状态" in body
    assert "incoming 文件 0" in body
    assert "等待本地授权原件" in body
...
def test_research_gap_dashboard_without_intake_report_keeps_tracked_target_count(tmp_path, monkeypatch):
    """公开 checkout 缺少本机报告时，不能把跟踪目标误报为零。"""
    import app

    monkeypatch.setattr(
        app,
        "AUTHORIZED_ORIGINAL_INTAKE_REPORT_PATH",
        tmp_path / "missing-intake-report.json",
    )
    body = app.research_gaps_page().decode("utf-8")
    assert "目标 2" in body
    assert "尚未生成本机接收报告" in body
    assert "/domestic/intake" in body
...
def test_1947_research_packet_carries_event_source_map_without_body():
    ...
    assert len(source_map["sources"]) == 16
    renmin_route = next(
        source for source in source_map["sources"]
        if source["source_id"] == "renminribao-1947-11-04-mirror-page1"
    )
    assert renmin_route["source_role"] == "public_periodical_scan_route"
    assert renmin_route["evidence_level"] == "L1"
    assert renmin_route["page_records"] == []
    assert renmin_route["identity_status"] == "public_mirror_page_verified_pending_local_acquisition"
    assert renmin_route["source_url"].startswith("https://cn.govopendata.com/renminribao/")
    naa_route = next(
        source for source in source_map["sources"]
        if source["source_id"] == "naa-tw-1947-11-15-illegal-league-reaction"
    )
    assert naa_route["source_role"] == "official_archive_image"
    assert naa_route["evidence_level"] == "L1"
    assert naa_route["page_records"] == []
    assert naa_route["identity_status"] == "local_image_saved_page_boundary_verified_pending_text_rights"
    assert naa_route["source_sha256"] == "95d533d10ab43ef4b5e01e9f723da3e5e56bfa31e2b7cf5ef27397268bb6d680"
    assert naa_route["local_acquisition"]["page_count"] == 1
    assert naa_route["local_acquisition"]["body_read"] is False
    assert naa_route["local_acquisition"]["ocr_performed"] is False
    assert naa_route["source_url"].startswith("https://aa.archives.gov.tw/")
    observer = next(
        source for source in source_map["sources"]
...
def test_academic_metadata_snapshot_fallback_without_staging(tmp_path, monkeypatch, db_missing_reason):
    ...
    assert snapshot["fulltext_readiness"]["fulltext_candidates"] == 17
    result = app._research_academic_matches(
        {"event_tags": ["1948"]},
