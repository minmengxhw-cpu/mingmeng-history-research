...
| 优先级 | 动作 | 收益 | 风险 |
|---|---|---|---|
| P0 | 修 FTS 中文 2 字检索（S2） | 中文人名/核心词可搜，检索价值质变 | 需重建索引，配合基线审计 |
| P0 | 隔离 1931 范围外 483 页（S1） | 检索净化、scope 合规 | 需确认无证据引用这 480 页 |
| P1 | 清理 64GB .bak 备份（S4） | 释放 60GB+ 磁盘 | 保留 2 个近期回滚点即可 |
| P1 | 候选断链修复（S3） | 采集真实闭合：补采 296 全文 / 降级登记 | 工作量集中在补采 |
| P2 | 来源体系统一（S5）+ registry 字段修复（S6） | 来源可反查、纠错可传导 | schema 迁移需回归 |
| P2 | QC 口径修正（S8）+ evidence_units 分级（S7） | 报告可信、人工复核聚焦真实异常页 | 低 |
| P3 | date 规范化（S9）+ 脚本收敛归档（S10）+ 小瑕疵清理（S11） | 整洁、可维护 | 低 |
